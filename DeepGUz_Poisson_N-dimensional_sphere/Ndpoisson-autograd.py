...
def preTrain(model,device,params,preOptimizer,preScheduler,fun):
    model.train()
    file = open(f"Error_Poisson_Penalty_dim={params["d"]}_Penalty={params["penalty"]}_SEED={params["SEED"]}.txt","w")
...
def train(model,device,params,optimizer,scheduler):
    ...
    for step in range(params["trainStep"]-params["preStep"]):
        ...
        if step%params["writeStep"] == params["writeStep"]-1:
            ...
            file = open(f"Error_Poisson_Penalty_dim={params["d"]}_Penalty={params["penalty"]}_SEED={params["SEED"]}.txt","a")
...
def main(seed = 1, dimension_N = 8):
    ...
    file = open(f"TIME_Poisson_Penalty.txt","a")
