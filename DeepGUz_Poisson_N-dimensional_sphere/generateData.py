...
# Sample from Dd-ball
def sampleFromDiskD(r,n,D):
    """
    r -- radius;
    n -- number of samples;
    D -- dimension.
    """
    array = np.random.normal(size=(n,D))
    norm = np.linalg.norm(array,2,axis=1)
    # print(np.min(norm))
    if np.min(norm) == 0:
        return sampleFromDiskD(r,n,D)
    else:
        array = np.multiply(array.T,1/norm).T
        radius = np.random.rand(n,1)**(1/D)
        array = np.multiply(array,radius)

        return r*array

def sampleFromSurfaceD(r,n,D):
    """
    r -- radius;
    n -- number of samples;
    D -- dimension.
    ...
    # print(np.min(norm))
    if np.min(norm) == 0:
        return sampleFromSurfaceD(r,n,D)
    else:
        array = np.multiply(array.T,1/norm).T
        return array*r
