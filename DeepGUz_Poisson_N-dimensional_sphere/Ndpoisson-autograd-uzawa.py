...
def train(model,device,params,optimizer,scheduler):
    ...
    data3 = torch.from_numpy(generateData.sampleFromSurfaceD(params["radius"],params["bdryBatch"],params["d"])).float().to(device)
...
def optim(step = 100, dimension_N = 8):
    # Parameters
    ...
    params["decay"] = 0.0001
    params["rho"] = 0.1
    params["uzawaStep"] = step
    params["trainStep"] = 500*step
...
def main(seed = 1, dimension_N = 8):
    ...
    preTrain(model,device,params,preOptimizer,None,rough)
    train(model,device,params,optimizer,scheduler)
    file = open(f"TIME_Poisson_Uzawa.txt","a")
